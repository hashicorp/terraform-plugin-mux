...
func TestMuxServerStateStore_UnlockState(t *testing.T) {
	t.Parallel()
	...
	if !testServer1.UnlockStateCalled["test_statestore_server1"] {
		t.Errorf("expected test_statestore_server1 UnlockState to be called on server1")
	}

	if testServer2.UnlockStateCalled["test_statestore_server1"] {
	...
	if !testServer2.UnlockStateCalled["test_statestore_server2"] {
		t.Errorf("expected test_statestore_server2 UnlockState to be called on server2")
	}

	if testServer1.UnlockStateCalled["test_statestore_server2"] {
		t.Errorf("unexpected test_statestore_server2 UnlockState called on server1")
	}
}
